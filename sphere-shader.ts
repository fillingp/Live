...
vec3 calc( vec3 pos ) {
  ...
  return pos +
    // User input distortion (more chaotic cosine wave on z-axis)
    1. * inputData.x * inputData.y * dir * (.5 + .5 * cos(inputData.z * pos.z + time * 1.5)) +
    // AI output distortion (smoother sine wave on y-axis)
